...
class RLAgent(Agent):
    def __init__(self,content_manager, path_to_model,
                 path_to_dstc, binarizers, max_turn, need_sl=True, rounds=1, batch_size=32
                 ):
        ...
        self.model = DoubleDQN.load(path_to_model) if os.path.exists(path_to_model) \
                           ...
                           memory_size = 200, batch_size=batch_size)
    ...
    def update_state_user(self, user_actions, nl=None, goal=None):
        ...
        r = self.reward(user_actions, goal)
    ...
    def state2vec(self, variants = None):
        ...
        result.append(int(len(variants)>0))
        result.append(len(variants))
    ...
    def reward(self, user_actions, goal):
        ...
        result = 0
        result += len(goal['current_filled_slots']) * 5
    ...
    def next(self):
        ...
        variants = self.content_manager.available_results(self.inform_slots, self.slot_restrictions)
        index = self.model.choose_action(self.state2vec(variants))
        ...
        for part in act_full.split('__'):
            ...
            if len(variants)>0:
                ...
                slot_value = self.inform_slots.get(slot_name, '')
